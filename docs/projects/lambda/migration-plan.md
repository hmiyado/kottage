...
| 経路 | 中央値 |
|---|---|
| 現行EC2（API GW → プロキシLambda → EC2 → TiDB） | **1,328 ms** |
| Lambda（Lambda → TiDB、ウォーム） | **570 ms** |

**Lambdaの方が半分以下に速い。** プロキシLambdaの中継が1段消えることと、
t2.nano（512MB・CPUクレジット制）よりLambdaのCPUが速いことによる。
...
| 指標 | 評価 |
|---|---|
| コールドスタート6秒 | ⚠️ 唯一の劣化点。ただし影響を受けるのは「しばらくアクセスが無かった後の最初の1リクエスト」のみ |
| ウォーム時レイテンシ | ✅ 現行の半分以下（1,328 → 570 ms） |
| コスト | ✅ 無料枠内 |
| 削減額 | ✅ $8.6/月（年$103） |

コールドスタート以外のすべての指標で現行を上回る。案B（Lightsail）は
コールドスタートが無い代わりに、レイテンシ改善も課金削減幅も劣る。
したがってLambdaで進める。
