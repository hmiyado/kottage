Feature: comments
  ...
  Scenario: create and delete comment
    ...
    * def serialNumber = response.serialNumber
    ...
    And method POST
    Then status 201
    And match response == {id: '#number', name: 'admin', body: 'new comment', createdAt: '#string', author: {id: '#number', screenName: '#string'}}
    ...
    * call read('classpath:kottage/users/share.feature@signOut')
    ...
    And method POST
    Then status 201
    And match response == {id: '#number', name: 'Taro', body: 'new comment', createdAt: '#string', author: '#notpresent'}
